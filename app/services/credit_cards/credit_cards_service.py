...
class CreditCardsService(CreditCardsServicer):
    def table(self, request, context):
        ...
        is_auth(metadata['auth_token'], '04_credit_cards_table')
    ...
    def get_all(self, request, context):
        metadata = dict(context.invocation_metadata())
        is_auth(metadata['auth_token'], '04_credit_cards_get_all')
    ...
    def get(self, request, context):
        try:
            metadata = dict(context.invocation_metadata())
            is_auth(metadata['auth_token'], '04_credit_cards_get')
    ...
    def save(self, request, context):
        try:
            metadata = dict(context.invocation_metadata())
            is_auth(metadata['auth_token'], '04_credit_cards_save')
    ...
    def update(self, request, context):
        try:
            metadata = dict(context.invocation_metadata())
            is_auth(metadata['auth_token'], '04_credit_cards_update')
            credit_cards_object = MessageToDict(request)
    ...
    def delete(self, request, context):
        try:
            metadata = dict(context.invocation_metadata())
            is_auth(metadata['auth_token'], '04_credit_cards_delete')
            card_credit = CreditCards.objects.get(id=request.id)
