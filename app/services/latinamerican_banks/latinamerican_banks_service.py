...
class LatinAmericanBanksService(LatinAmericanBanksServicer):
    def table(self, request, context):
        ...
        is_auth(metadata['auth_token'], '04_latin_american_banks_table')
    ...
    def get_all(self, request, context):
        metadata = dict(context.invocation_metadata())
        is_auth(metadata['auth_token'], '04_latin_american_banks_get_all')
    ...
    def get(self, request, context):
        try:
            metadata = dict(context.invocation_metadata())
            is_auth(metadata['auth_token'], '04_latin_american_banks_get')
    ...
    def save(self, request, context):
        try:
            metadata = dict(context.invocation_metadata())
            is_auth(metadata['auth_token'], '04_latin_american_banks_save')
    ...
    def update(self, request, context):
        try:
            metadata = dict(context.invocation_metadata())
            is_auth(metadata['auth_token'], '04_latin_american_banks_update')
            latinamerican_banks_object = MessageToDict(request)
    ...
    def delete(self, request, context):
        try:
            metadata = dict(context.invocation_metadata())
            is_auth(metadata['auth_token'], '04_latin_american_banks_delete')
            latin_banks = LatinAmericanBanks.objects.get(id=request.id)
